/*
  This Arduino code connects an ESP32 to a Wi-Fi network and Firebase to monitor vehicle fuel levels.
  
  - **Fuel Level Monitoring**: Utilizes an HC-SR04 ultrasonic sensor to measure the distance to fuel, indicating fuel level, 
    and detects theft or refueling events by comparing previous and current readings.
  
  - **Firebase Integration**: Sends real-time alerts and data updates to a Firebase database, 
    including fuel level, distance measurements, and alert status.
  
  - **Alert System**: Triggers alerts for fuel theft or refueling based on significant changes in measured distance, 
    updating the Firebase database accordingly.
  
  - **NTP Time Synchronization**: Retrieves the current date and time from an NTP server 
    to timestamp alerts accurately.
*/
